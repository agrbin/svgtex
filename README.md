...
Update node modules
--------------------
To rebuild the packages you have rename Makefile.ignore to Makefile and run
```make clean && make ``` afterwords undo the renaming and run
```dpkg-buildpackage -uc -us```. This will create a file called mathoid_*.deb
that you can install via ```sudo dpkg -i mathoid_*.deb```
